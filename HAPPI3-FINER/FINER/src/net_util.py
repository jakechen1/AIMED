...
def read_net(file_name, node_size, gene_list):
    ...
    while True:
        ...
        if node1 not in gene_iso_dict or node2 not in gene_iso_dict:
            continue
    ...
    print("# of test edges:", len(test_set_edges))
    return G, list(set(genes_with_edges)), test_set_edges
...
def create_neg_table(G):
    print('Creating negative sampling table...')
    all_nodes = []
    for node in G.nodes():
        all_nodes.append(node)
    power = 0.75
    norm = sum([math.pow(G.degree(node), power) for node in all_nodes]) # Normalizing constant
