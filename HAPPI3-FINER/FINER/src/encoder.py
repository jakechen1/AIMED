...
class Encoder(object):
    ...
    def apply_loss_function_emb(self, global_step):
        ...
        self.emb_loss = 1.0 * nb_loss + 1.0 * lp_loss + 1.0 * coexp_loss + self.lambda_func * func_loss
    ...
    def apply_loss_function_w(self, global_step):
        ...
        self.lp_loss = self.linkpred_loss(
            self.embWemb_2,
            self.link_weights) + 0.1 * tf.nn.l2_loss(self.W - 1) + self.lambda_func * func_loss
        self.lr = tf.compat.v1.train.exponential_decay(
            self.learning_rate,
            global_step,
            self.decay_step,
    ...
    def train(self, co_exp_net, instance_to_bag, cur_iter, gene_list, iso_list,
              y_train, y_test, func_feature, last_p):
        print("train encoder...")
        ...
        n_table = net_util.create_neg_table(self.ppi_net)
        nodes_with_edges = []
        for node in self.iii_net.nodes():
            ...
            for i in range(len(batch_indexes)):
                ...
                for idx in batch_indexes[i]:
                    ...
                    nolink_sample = self.neg_sample(
                        n_table, self.negative * self.positive)
        ...
        self.iii_net, self.neighbor_changed, pred_prob, nodes_idx, last_p = self.predict_link(
            cur_iter, embeddings, W, gene_list, y_train, y_test, self.iii_net, last_p)

        return self.iii_net, pred_prob, nodes_idx, last_p
    ...
    def predict_link(self, iter, embeddings, W, gene_list,
        ...
        predicted_links_p = embeddings[nodes_idx, :].dot(np.diag(W)).dot(
            np.transpose(embeddings[nodes_idx, :]))
        predicted_links_p = 1.0 / (1.0 + np.exp(-predicted_links_p))
        predicted_links = 0.7 * last_p + 0.3 * predicted_links_p
        last_p = predicted_links

        all_link_weights = predicted_links.flatten()
        par = int(len(all_link_weights) * 0.002)
        link_thresh = all_link_weights[np.argpartition(all_link_weights, -par)[-par]]
        ...
        for node in G.nodes():
            if set(G.neighbors(node)) != set(self.iii_net.neighbors(node)):
                #print(node, list(G.neighbors(node)), list(self.iii_net.neighbors(node)))
                neighbor_changed[node] = 1
        print("Links of %d nodes were updated." % np.sum(neighbor_changed))

        return G, neighbor_changed, predicted_links_p, nodes_idx, last_p
